let cmn={},ssd={post:[]},IS_HISTORY_API=(cmn.saveFormData=t=>{var s=document.querySelector(t);if(s){s=new FormData(s);let e={};s.forEach((t,s)=>{e[s]=t}),sessionStorage.setItem(t,JSON.stringify(e))}},cmn.loadFormData=t=>{let a=document.querySelector(t);if(a){t=sessionStorage.getItem(t);if(t){let e=JSON.parse(t);Object.keys(e).forEach(s=>{var t=a.querySelector(`[name="${s}"]`);t&&("checkbox"===t.type||"radio"===t.type?a.querySelectorAll(`[name="${s}"]`).forEach(t=>{t.checked=t.value===e[s]}):"SELECT"===t.tagName&&Array.isArray(e[s])?Array.from(t.options).forEach(t=>{t.selected=e[s].includes(t.value)}):t.value=e[s])})}}},cmn.getAgeAtThatTime=(t,s="1975-07-09")=>{let e=t,a=s,o={},n=new Date,i=n.getFullYear(),r=n.getMonth()+1,c=n.getDate(),l;return e=e.replace(/-/g,""),a=a.replace(/-/g,""),r<10&&(r="0"+r),c<10&&(c="0"+c),l=i+""+r+c,0<e-a&&(o.age=Math.floor((e-a)/1e4),o.is_same=Math.floor((l-a)/1e4)===o.age),o},cmn.getDir=t=>{let s=t.split("/").filter(Boolean),e={};return 0===s.length?e.level1="home":s.forEach((t,s)=>{e["level"+(s+1)]=t}),e},cmn.getNrToBr=(t="")=>t=(t=(t=t.replace(/\r\n/g,"<br>")).replace(/\r/g,"<br>")).replace(/\n/g,"<br>"),cmn.getParam=t=>{let s=t||location.href,e=[],a={},o;for(o in s=1<s.split("?").length?s.split("?")[1]:s,e=s.split("&")){var n=e[o].split("=");2==n.length&&""!=n[1].trim()&&(a[n[0]]=n[1])}return a},history&&history.pushState&&void 0!==history.state),IS_HISTORY_PUSH=!1,CURRENT_TIME=(cmn.historyPushState=(t,s,e)=>{IS_HISTORY_API&&(history.pushState(t,s,e),IS_HISTORY_PUSH=!0)},cmn.historySet=s=>{IS_HISTORY_API&&$(window).on("popstate",function(t){t=t.originalEvent?.state||null;s&&s(t)})},$(document).on("mouseenter","a, button",function(){$(this).addClass("hover")}),$(document).on("mouseleave","a, button",function(){$(this).removeClass("hover")}),cmn.htmlEntityDecode=t=>{var s=document.createElement("textarea");return s.innerHTML=t,s.value},cmn.htmlEscape=(t="")=>{var s=document.createElement("div");return s.textContent=t,s.innerHTML},cmn.loadHtml=async(t="")=>{if(""===t.trim())return{status:"fail",message:"ファイル名が指定されていません"};try{var s=await fetch(t,{method:"GET",headers:{"X-CSRF-Token":CSRF_TOKEN}});if(s.ok)return await s.text();throw new Error("HTTPエラー: "+s.status)}catch(t){return""}},cmn.loadImg=t=>{$("img",t).each(function(){let t=$(this),s=t.attr("data-src");var e=new Image;e.onload=()=>{t.attr("src",s).removeAttr("data-src")},e.src=s})},cmn.loadJson=async(t="")=>{if(""===t.trim())return{status:"fail",message:"ファイル名が指定されていません"};try{var s=await fetch("/api/index.php",{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-Token":CSRF_TOKEN},body:JSON.stringify({filename:t})});return s.ok?await s.json():{status:"fail",message:"HTTPエラー: "+s.status}}catch(t){return{status:"fail",message:t.message||"不明なエラーが発生しました"}}},cmn.pageScroll=(t=1,s=300,e)=>($("html, body").stop(),$("html").animate({scrollTop:t},s),$("body").animate({scrollTop:t},s,function(){e&&e()}),!1),(new Date).getTime()),scroll=(cmn.removeSplash=()=>{var t=1500-Math.min((new Date).getTime()-CURRENT_TIME,1500);setTimeout(()=>{$(".js-splash").fadeOut(()=>{$("body").addClass("show")})},t)},cmn.replaceTag=(t="")=>t.replace(/<[^>]*>/g,""),cmn.replaceUTF=(t="")=>t.replace(/u([\da-fA-F]{4})/g,"&#x$1;"),{}),IS_LOCALSTORAGE=(scroll.is_touch=!!("ontouchstart"in window&&"ontouchend"in window||navigator.msPointerEnabled||0<navigator.maxTouchPoints),scroll.eventstart=scroll.is_touch?"touchstart":"mousedown",scroll.eventmove=scroll.is_touch?"touchmove":"mousemove",scroll.eventend=scroll.is_touch?"touchend":"mouseup",scroll.isNumber=(t="")=>/^[0-9]+$/.test(t),(()=>{let t=new $.Event("resizestop"),s;$(window).on("resize",()=>{s&&clearTimeout(s),s=setTimeout(function(){$(window).trigger(t)},300)})})(),scroll.show=(e=".scroll",a={})=>{let o=$(e),t=$(".scroll__outer",o),n=$(".scroll__inner",o),i=$(".scroll__content",o),s=$(".scroll__bar",o);if(scroll.is_touch)return t.width("").height(""),!1;if(0===o.length||0===t.length||0===n.length||0===i.length)return!1;let r=a.direction||"horizontal",c="horizontal"===r,l="horizontal"===r?"left":"top",m="horizontal"===r?"width":"height",p="horizontal"===r?"height":"width",d="horizontal"===r?"margin-top":"margin-left",h="";a.bg&&(h+=e+" .scroll__bar::after{background:"+a.bg+"}",delete a.bg),a.color&&(h+=e+" .scroll__bar span::after{background:"+a.color+"}",delete a.color),a.size&&scroll.isNumber(a.size)&&(h+=e+" .scroll__bar span::after{"+p+":"+Math.min(a.size,20)+"px;"+d+": "+Math.min(a.size,20)/2*-1+"px}",delete a.size),a.bg_size&&scroll.isNumber(a.size)&&(h+=e+" .scroll__bar::after{"+p+":"+Math.min(a.bg_size,20)+"px;"+d+": "+Math.min(a.bg_size,20)/2*-1+"px}",delete a.bg_size),""!=h&&$("head").append("<style>"+h+"</style>"),n.css({overflow:"hidden"});var u=i[0].scrollWidth,g=i[0].scrollHeight;let f=c?o.width():o.height(),_=c?u:g;if(!(1<=Math.abs(f-_)&&1<_-f))return 0<s.length&&s.hide(),!1;s.show(),n.css({overflow:"auto"});let v=!1,b=0,w=0,y=0,S,j=0,T=0,k=0,P=!1;f-=a.padding||0;n.on("scroll",()=>{if(v)return!1;var t=c?i.position().left:i.position().top;let s=Math.ceil(Math.abs(t)/_*100);100<s+T&&(s=100-T),C.css(l,s+"%")}),$(window).on("resize",()=>{P=!0}),$(window).on("resizestop",()=>{var t=$(".scroll__bar-control",o),s=c?t.width():t.height();S=c?window.innerWidth:window.innerHeight,y!=S&&(scroll.show(e,a),0<t.length&&(j=f-s),P=!1,y=S)}),0===s.length&&(o.append(`
    ...
    `),s=$(".scroll__bar",o),a.padding)&&s.css(m,"calc(100% - "+a.padding+"px)").css(l,a.padding/2+"px");let C=$(".scroll__bar-control",o);c?(t.height(g),n.height(g+30)):(t.width(u),n.width(u+30)),T=Math.ceil(f/_*100),k=c?(C.width(T+"%"),C.width()):(C.height(T+"%"),C.height()),j||(j=f-k,(()=>{let s;C.on(scroll.eventstart,function(t){v=!0,s=c?i.position().left:i.position().top,b=c?t.pageX:t.pageY,w=Math.floor(Math.abs(s)/_*f)}),$(document).on(scroll.eventmove,function(e){if(v&&!P){e=(c?e.pageX:e.pageY)-b,e=w+e;let t=0,s;e=Math.max(0,e),e=Math.min(Math.ceil(j),e),t=Math.ceil(e/f*100),s=_*(t/100),100<t+T&&(t=100-T),C.css(l,t+"%"),c?n.scrollLeft(s):n.scrollTop(s)}}),$(document).on(scroll.eventend,function(){v=!1})})())},cmn.setScriptFromText=t=>{t=document.querySelector(t);t&&t.querySelectorAll("script").forEach(t=>{var s=document.createElement("script");t.src?s.src=t.src:s.textContent=t.textContent,document.body.appendChild(s),t.remove()})},cmn.shuffleArray=(t=[])=>{let s,e;for(s=t.length;1<s;s--)e=Math.floor(Math.random()*s),[t[e],t[s-1]]=[t[s-1],t[e]];return t},cmn.sticky=(t="",s,e)=>{let a=document.querySelector(t);if(""===t.trim()||!a||"function"!=typeof s||"function"!=typeof e)return!1;let o=a.getBoundingClientRect().top;window.addEventListener("scroll",()=>{var t=a.getBoundingClientRect().top;t===o?s():o,o=t})},"localStorage"in window&&null!==window.localStorage),IS_SESSIONSTORAGE="sessionStorage"in window&&null!==window.sessionStorage;cmn.storageLS=(s,t)=>{let e=null;if(IS_LOCALSTORAGE&&void 0!==s)if(void 0!==t)localStorage[s]="object"==typeof t?JSON.stringify(t):t;else try{e=new Function("return "+localStorage[s])()}catch(t){e=localStorage[s]}return e},cmn.storageDelLS=t=>{localStorage.removeItem(t)},cmn.storageSS=(s,t)=>{let e=null;if(IS_SESSIONSTORAGE&&void 0!==s)if(void 0!==t)sessionStorage[s]="object"==typeof t?JSON.stringify(t):t;else try{e=new Function("return "+sessionStorage[s])()}catch(t){e=sessionStorage[s]}return e},cmn.storageDelSS=t=>{sessionStorage.removeItem(t)},$(()=>{let l=!1;$(document).on("click",".js-comment-submit",async function(){let t=$(this),s=t.closest(".js-form-comment"),e=$('[name="comment"]',s),a=$('[name="author"]',s),o=$(".js-comment-notification",s),n=$(".js-comment-inner"),i=$(".js-comment-list");$('[name="comment_post_ID"]',s).val();var r=new FormData(s[0]),r=Object.fromEntries(r.entries());if(e.on("input",()=>{o.removeClass("caution").hide()}),!l){if(""===e.val().trim())return o.addClass("caution").html("コメントが入力されていることをご確認の上、もう一度お試しください。").show(),!1;l=!0,t.html('<span class="animation-blinker">送信中...</span>');var c,r=await(await fetch("/assets/ajax/post-comment.php",{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-Token":CSRF_TOKEN},body:JSON.stringify(r)})).text();"success"===r?(o.removeClass("caution").html("コメントありがとうございます！").show(),c=`
        ...
        `,0===i.length&&(n.append('<div class="post-comment__list js-comment-list"></div>'),i=$(".js-comment-list")),0<$(".post-comment__unit",i).length?$(".post-comment__unit:first-child",i).before(c):i.append(c),e.val(""),a.val(""),console.log(ssd.post)):(console.log(r),o.addClass("caution").html("コメント送信できませんでした。恐れ入りますが、もう一度お試しください。").show()),l=!1,t.html("コメントする")}return!1})}),ssd.changePage=async(g={},f)=>{let _=$(".js-article-inner"),v=$(".js-loading");_.animate({top:-10,opacity:0},350,async function(){$(window).scrollTop(0),v.show(),g.path||(g.path="404");var t="https://simplesimples.com"+g.path,s=g.path.replace(/^\/|\/$/g,"").split("/"),e={};let a="シンプルシンプルデザイン",o="",n="",i,r,c="",l=!1,m=!1,p=!1,d=!1;switch(s[0]){case"":d=!0,a="",o="",r=await fetch("/assets/include/pages/home.php"),c=await r.text();break;case"blog":case"moviereview":a="個人ブログは引越ししました | "+a,o="個人ブログは別ドメインに引っ越ししました。ぜひ、引っ越し先をご覧ください。",r=await fetch("/assets/include/pages/move.php"),c=await r.text();break;case"service":a="サービス | "+a,o="",r=await fetch(`/assets/include/pages/${s[0]}.php`),c=await r.text();break;case"about":a="事業内容 | "+a,o="",r=await fetch(`/assets/include/pages/${s[0]}.php`),c=await r.text();break;case"contact":p=!0,a="お問い合わせ | "+a,o="",r=await fetch(`/assets/include/pages/${s[0]}.php`),c=await r.text();break;case"privacy":a="プライバシーポリシー | "+a,o="",r=await fetch(`/assets/include/pages/${s[0]}.php`),c=await r.text();break;case"post":a="制作ブログ | "+a,o="Web制作や映像制作を主軸とした備忘録です。",n="/assets/images/post-bg-searchbox.png",r=await fetch(`/assets/include/pages/${s[0]}.php`),c=await r.text();break;case"search":var h,u=cmn.getParam(location.search).q;u?(u=decodeURIComponent(u),h=0<(h=(r=await ssd.loadPostSearch(u)).count||0)?h+"件見つかりました。":"見つかりませんでした。",u=cmn.htmlEscape(u),a=`「${u}」の検索結果 | 制作ブログ | `+a,o=`制作ブログ内で「${u}」を含む記事は、`+h,n="/assets/images/post-bg-searchbox.png",c=ssd.setPostListSearch(r.ids||[],o),c=ssd.htmlSeachForm(u)+c):l=!0;break;case"web":case"video":case"overview":let t="";"web"===s[0]&&s[3]?t=s[3]:"web"!=s[0]&&s[2]&&(t=s[2]),t?(m=!0,t=decodeURIComponent(t),(h=ssd.getPostData(t))&&(a=h.title+" | "+a,o=h.description,n=h.img,c=await ssd.getPostHtml(h.id))&&""!=c?i={id:h.id,html:c}:l=!0):(a=" | "+a,o="",n="",c=ssd.setPostCategory(s[s.length-1])),c=ssd.htmlSeachForm()+c;break;default:l=!0}if(ssd.loading_page!=g.path)return console.error("Diff Page",ssd.loading_page,g.path),!1;switch(l&&(a="表示できませんでした | "+a,o="",r=await fetch("/assets/include/pages/system.php"),c=await r.text()),$("title").html(a),$('meta[name="description"]').attr("content",o),$('meta[property="og:image"]').attr("content",n),$('link[rel="canonical"]').attr("href",t),$("body").attr("data-menu","hide").attr("data-page",s[0]||"home"),v.hide(),_.html(c),_.animate({top:0,opacity:1},350),!0){case m:hljs.highlightAll(),ssd.setPostLink(),cmn.setScriptFromText(".js-post-content");break;case d:ssd.setBusinessquotes();break;case p:cmn.loadFormData(".js-form-contact")}f&&g.path!=ssd.last_history&&(ssd.last_history=g.path,e.path=g.path,i&&(e.data=i),cmn.historyPushState(e,"",g.path)),ssd.is_delete_splash||(ssd.is_delete_splash=!0,cmn.removeSplash())})},$(()=>{let l=!1,m=t=>{$('.js-form-contact button[type="button"]').before('<p class="contact__error js-contact-error">'+t+"</p>")};$(document).on("click",'.js-form-contact button[type="button"]',async function(){let t=$(this),s=t.closest(".js-form-contact"),a=$(".js-contact-error",s),o=$(".js-contact-success",s),n=$(".js-contact-success-hide",s);if($(".js-link-contact-noreply",s).on("click",()=>(n.toggle(),!1)),!l){a.remove(),o.hide();var i=new FormData(s.get(0));let e={};if(i.forEach((t,s)=>{e[s]=t}),""===e.name.trim())return m("お名前を入力してください。"),!1;if(!/\S+@\S+\.\S+/.test(e.email))return m("メールアドレスを入力してください。"),!1;if(""===e.content.trim())return m("お問い合わせ内容を入力してください。"),!1;l=!0,t.html('<span class="animation-blinker">送信中...</span>');var i=e.email,r=`
<p>【こちらは自動返信メールです】</p>
...
${e.content.trim()}
            `,i=await(await fetch("/assets/ajax/contact-form.php",{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-Token":CSRF_TOKEN},body:JSON.stringify({email:i,subject:"【シンプルシンプルデザイン】お問い合わせありがとうございます",body:r,altbody:c})})).json();if("fail"===i.status)switch(i.message){case"Reload_due_to_invalide_request":m("送信できませんでした。");break;case"Error Message body empty":m("入力不備により、送信できませんでした。")}else n.hide(),o.show(),$('[name="name"]').val(""),cmn.storageDelSS(".js-form-contact");l=!1,t.html("送信する")}return!1}),$(document).on("change",".js-form-contact input, .js-form-contact textarea, .js-form-contact select",()=>{cmn.saveFormData(".js-form-contact")})}),ssd.getPostData=(s="")=>ssd.list.find(t=>decodeURIComponent(t.slug.replace(/^"|"$/g,""))===s)||null,ssd.getPostHtml=async s=>{let t=ssd.post.find(t=>t.id==s);if(!t){if(!(t=await ssd.loadPost(s))?.content)return"TODO ネット切れてる場合";ssd.post.push(t)}var e=t?.image?`<figure class="post__mainvisual" style="background-image: url(${t.image});"></figure>`:"";let a=t?.acf?.update||"";a&&(n=a.split("-"),a=`(${n[0]}年${+n[1]}月${+n[2]}日更新)`);var o,n=encodeURIComponent("https://simplesimples.com"+t.link),i=encodeURIComponent(cmn.htmlEntityDecode(t.title)),r=encodeURIComponent("【シンプルシンプルデザイン】"),r=`
<ul class="sns-list post__sns-list">
  ...
  `,i=t?.acf?.stylesheet?`<style>${t.acf.stylesheet}</style>`:"",n=t?.acf?.javascript?`<script>${t.acf.javascript}</script>`:"",c=0===t.comments.length?'<p class="post-comment__none js-comment-none">この記事の最初のコメントを書いてくれると嬉しいです！</p>':"";let l="";for(o of t.comments){let{author:t,content:s,date:e}=o;t=t.trim()||"(匿名)",s=(s=s.replace(/</g,"＜").replace(/>/g,"＞")).replace(/&lt;/g,"＜").replace(/&gt;/g,"＞"),s=cmn.getNrToBr(s),l+=`
<div class="post-comment__unit">
  <div class="post-comment__unit-inner">
    ${s}
    <div class="post-comment__unit-info">
    ...
    `}return l=0===t.comments.length?"":`<div class="post-comment__list js-comment-list">${l}</div>`,`
${i}
